...
TEST(CApiRobot, MotorPresets) {
    ...
    EXPECT_FLOAT_EQ(p.stall_torque_newton_meters, 7.09f);
    EXPECT_EQ(p.count, 2);
    EXPECT_EQ(frcsim_motor_params_preset(99, 1, &p), FRCSIM_ERR_INVALID_ARGUMENT);
    EXPECT_EQ(frcsim_motor_params_preset(FRCSIM_MOTOR_MINION, 0, &p), FRCSIM_ERR_INVALID_ARGUMENT);
}
...
TEST(CApiRobot, AddDriveAndReadSharedIo) {
    ...
    EXPECT_FLOAT_EQ(config.modules[1].y_meters, -0.275f) << "front-right module";
    ...
    EXPECT_NEAR(io->x_meters, 2.0, 1e-5);
    EXPECT_NEAR(io->yaw_radians, 0.5, 1e-5);
    EXPECT_DOUBLE_EQ(io->gyro_yaw_radians, io->yaw_radians) << "sensors are ideal by default";
    ...
    for (uint32_t m = 0; m < io->module_count; ++m) {
        io->modules[m].drive_command_volts = 6.0f; // modules at angle 0 drive along robot +X (yaw 0.5 rad)
    }
    for (int i = 0; i < 50; ++i) {
    ...
    const double speedMetersPerSec = std::hypot(io->vx_meters_per_sec, io->vy_meters_per_sec);
    EXPECT_GT(speedMetersPerSec, 1.5);
    EXPECT_NEAR(std::atan2(io->vy_meters_per_sec, io->vx_meters_per_sec), 0.5, 0.05) << "drives along its heading";
    EXPECT_GT(io->modules[0].drive_rotor_position_radians, 10.0);
    EXPECT_GT(io->modules[0].drive_rotor_velocity_rad_per_sec, 100.0f);
    EXPECT_NEAR(io->modules[0].drive_applied_volts, 6.0f, 1e-3f);
    EXPECT_GT(io->modules[2].normal_force_newtons, 100.0f);
    EXPECT_LT(io->battery_volts, 12.5f);
    EXPECT_EQ(io->brownout, 0u);

    ASSERT_EQ(frcsim_robot_reset_pose(world.get(), robot, 5.0f, 5.0f, 0.0f), FRCSIM_OK);
    EXPECT_NEAR(io->x_meters, 5.0, 1e-5);
    EXPECT_NEAR(io->vx_meters_per_sec, 0.0f, 1e-5f);
}
...
TEST(CApiRobot, InvalidInputsAreRejected) {
    ...
    frcsim_robot_io(world.get(), robot)->modules[0].drive_command_volts = NAN;
