...
TEST(KinematicBodies, MoveToReachesTargetAfterStep) {
    ...
    world.step(0.020, 5);
    EXPECT_NEAR(world.kinematics().position(k).GetX(), 0.06f, 1e-4f);
}
...
TEST(KinematicBodies, PlowPushesRestingPiece) {
    ...
    const std::uint32_t piece = world.pieces().spawnOne(fuel, JPH::Vec3(1.0f, 0, test::kFuelRadius));
    ...
    EXPECT_GT(world.pieces().position(piece).GetX(), x + 0.45f)
